...
export function Sidebar({ username }: { username: string, email: string }) {
  ...
  return (
    <aside className={`${isCollapsed ? 'w-20' : 'w-64'} flex-shrink-0 bg-background/60 backdrop-blur-xl border-r border-white/5 h-full flex flex-col pb-6 pt-6 px-4 transition-all duration-300 relative group/sidebar`}>
      ...
      {/* ── Unified Footer Block ── anchored to bottom */}
      <div className="mt-auto space-y-3">

        {/* Brand Signature */}
        <Link
          href="/dashboard"
          className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-2.5 px-2'} group py-2`}
        >
          <Image
            src="/logo.png"
            alt="OlympLab Logo"
          ...
          />
          {!isCollapsed && (
            <span className="font-mono font-bold text-sm tracking-tight text-muted-foreground group-hover:text-foreground transition-colors overflow-hidden whitespace-nowrap">
              OlympLab
            </span>
          )}
        </Link>
        ...
        <div className={`space-y-0.5 ${isCollapsed ? 'flex flex-col items-center' : ''}`}>
          ...
          >
            <div className={`flex-shrink-0 rounded-full bg-gradient-to-br from-secondary to-muted border border-white/5 text-foreground flex items-center justify-center text-[10px] font-bold w-8 h-8 ${!isCollapsed ? 'mr-3' : ''} group-hover/profile:border-primary/30 transition-colors`}>
          ...
          <Link
            ...
            className={`flex items-center ${isCollapsed ? 'justify-center w-10 h-10' : 'px-3 py-2'} rounded-md transition-all duration-200 group ${
          ...
          <button
            ...
            className={`flex items-center ${isCollapsed ? 'justify-center w-10 h-10 p-0' : 'w-full px-3 py-2'} rounded-md text-sm font-medium text-muted-foreground hover:text-destructive hover:bg-destructive/5 transition-all font-sans group`}
          >
