  ...
  return (
    <div className="not-prose my-8 p-6 rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h4 className="text-sm font-bold text-foreground uppercase tracking-wider">
            {isRu 
              ? `Визуализация: ${isStack ? 'Стек (LIFO)' : 'Очередь (FIFO)'}` 
              : `Visualization: ${isStack ? 'Stack (LIFO)' : 'Queue (FIFO)'}`
            }
          </h4>
          <p className="text-xs text-muted-foreground mt-1">
            {isStack 
              ? (isRu ? 'Last In, First Out — последний пришел, первый ушел' : 'Last In, First Out — last element in is first out')
              : (isRu ? 'First In, First Out — первый пришел, первый ушел' : 'First In, First Out — first element in is first out')
            }
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {isStack ? (isRu ? 'Извлечь (Pop)' : 'Pop') : (isRu ? 'Извлечь (Dequeue)' : 'Dequeue')}
          </button>
          <button
            onClick={push}
          ...
          >
            ...
            {isStack ? (isRu ? 'Добавить (Push)' : 'Push') : (isRu ? 'Добавить (Enqueue)' : 'Enqueue')}
          </button>
        </div>
      </div>
      ...
      <div className={cn(
        ...
        isStack ? "flex-col-reverse" : "flex-row"
      )}>
        <AnimatePresence mode="popLayout">
          {items.map((val, idx) => (
            <motion.div
              key={val}
              layout
              initial={{ opacity: 0, scale: 0.8, x: !isStack ? 50 : 0, y: isStack ? -50 : 0 }}
              ...
              className={cn(
                "w-16 h-16 flex items-center justify-center rounded-xl border font-mono font-bold text-lg shadow-lg bg-card relative",
                idx === items.length - 1 && isStack ? "border-amber-500 text-amber-500 shadow-amber-500/20" : 
                idx === 0 && !isStack ? "border-amber-500 text-amber-500 shadow-amber-500/20" : "border-primary/50 text-foreground"
              )}
            >
              {val}
              {(idx === items.length - 1 && isStack) && (
        ...
        {items.length === 0 && (
          <div className="text-muted-foreground font-mono text-sm italic opacity-50">
            {isRu ? 'Пусто...' : 'Empty...'}
          </div>
        )}
      </div>
